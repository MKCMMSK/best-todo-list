...
function getAPIToDo(query, location, callback) {
  getProbability(query, (err, api)=>{
    switch (api) {
      case "book":
        getBook(query, (err, book)=> { callback(null, null, book)});
        break;
      case "movie":
        getMovie(query, (err, movie)=> { callback(null, null, movie)});
        break;
      case "tv":
        getTvShow(query, (err, tv)=> { callback(null, null, tv)});
        break;
      case "product":
        getProduct(query, (err, product)=> { callback(null, null, product)});
        break;
      case "restaurant":
        getRestaurant(query, location, (err, location,restaurant)=> { callback(null, null, restaurant)});
    }
  });
}

function getProbability(query, callback) {
  request({
