...
let {getItems} = require('../../routes/helpers');
...
function getAPIToDo(query, location, callback) {
  getProbability(query, (err, api)=>{
    ...
    case "book":
      ...
      break;
    case "movie":
      getMovie(query, (err, movie)=> {
        callback(null, null, movie);
      });
      break;
    case "tv":
      getTvShow(query, (err, tv)=> {
        callback(null, null, tv);
      });
      break;
    case "product":
      getProduct(query, (err, product)=> {
        callback(null, null, product);
      });
      break;
    case "restaurant":
      getRestaurant(query, location, (err, location,restaurant)=> {
        callback(null, null, restaurant);
      });
    }
  });
