...
module.exports = (helpers, user) => {
  ...
  function addToDB(search, objObj, user, callback) { //add into db after finding the probable item, returns an object as well if wrong item
    getProbability(search, (err, api)=>{
      switch (api) {
      case "book":
        helpers.addBook(objObj.book, user);
        delete objObj.book;
        callback(null, null, objObj);
        break;
      case "movie":
        helpers.addMovie(objObj.movie, user);
        delete objObj.movie;
        callback(null, null, objObj);
        break;
      case "tv":
        helpers.addTvShow(objObj.tv, user);
        delete objObj.tv;
        callback(null, null, objObj);
        break;
      case "product":
        helpers.addProduct(objObj.product, user);
        delete objObj.product;
        callback(null, null, objObj);
        break;
      case "restaurant":
        helpers.addRestaurant(objObj.restaurant, user);
  ...
  function changeCategory(obj, callback) {
    console.log(obj);
    switch (obj.selected) {
      case "book":
        helpers.addBook(obj.book);
        break;
      case "restaurant":
        helpers.addRestaurant(obj.restaurant);
        break;
      case "product":
        helpers.addProduct(obj.product);
        break;
      case "movie":
        helpers.addMovie(obj.movie);
        break;
      case "tv":
        helpers.addTvShow(obj.tv);
    }
    console.log(obj.selected);
    callback(null,obj.selected);
  }
  ...
  return {getAPIToDo, addToDB, changeCategory};
};
